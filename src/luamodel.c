...
static int meta_find(lua_State* L) {
  ...
  lua_pushinteger(L, vlib.model_find(lua_checkmodel(L, 1, 0), luaL_checkstring(L, 2)));
  return 1;
}
...
static const luaL_Reg modelmeta[] = {{"recognizer", luavosk_newrecognizer},
                                     {"find", meta_find},
...
static int newmodel(lua_State* L) {
  ...
  if (lua_toboolean(L, 2)) {
    if (vlib.bmodel_new == NULL || (*ud = vlib.bmodel_new(modeln)) == NULL) return luaL_error(L, "Failed to initialize a batched model");
    luaL_setmetatable(L, metanm[1]);
  } else {
    if ((*ud = vlib.model_new(modeln)) == NULL) return luaL_error(L, "Failed to initialize a model");
...
void luavosk_model(lua_State* L) {
  ...
  if (vlib.bmodel_new) luahelp_newmt(L, metanm[1], bmodelmeta);
