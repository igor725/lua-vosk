...
static int loadvosk(lua_State* L) {
  if (lua_istable(L, 1)) {
    ...
    for (int i = 1; i <= tabsize; i++) {
      lua_rawgeti(L, 1, i);
      if (lua_isstring(L, -1) && luavosk_initlib(luaL_checkstring(L, -1))) return 0;
    }
  } else {
...
static lua_Integer _readfile(lua_State* L, void* buff, lua_Integer size, int idx) {
  int32_t const stackStart = lua_gettop(L);
...
static lua_Integer _seekfile(lua_State* L, lua_Integer offset, int origin, int idx) {
  ...
  lua_pushvalue(L, idx);
  switch (origin) {
    case SEEK_CUR: lua_pushliteral(L, "cur"); break;

    case SEEK_END: lua_pushliteral(L, "end"); break;

    case SEEK_SET: lua_pushliteral(L, "set"); break;
  }
...
static int wavguess(lua_State* L) {
  ...
  if (_readfile(L, &temp, 4, 1) != 4 || temp != 0x46464952 /*RIFF*/) return luaL_error(L, "Not a wave file");
  if (_seekfile(L, 4, SEEK_CUR, 1) != 0 || _readfile(L, &temp, 4, 1) != 4 || temp != 0x45564157 /*WAVE*/) return luaL_error(L, "WAVE id was not found");
  if (_readfile(L, &temp, 4, 1) != 4 || temp != 0x20746D66 /*fmt */) return luaL_error(L, "No format sub-chunk found");
  if (_readfile(L, &nfsz, 4, 1) != 4 || nfsz <= 0) return luaL_error(L, "Invalid format sub-chunk size");
  if (_readfile(L, &nfmt, 2, 1) != 2 || (nfmt != 0x0001 && nfmt != 0x0003)) return luaL_error(L, "Unsupported data format");
  if (_readfile(L, &nchs, 2, 1) != 2 || nchs > 1) return luaL_error(L, "Not a mono sound file");
  if (_readfile(L, &nsamp, 4, 1) != 4 || nsamp == 0) return luaL_error(L, "Failed to determine sample rate");
  if (_seekfile(L, 8, SEEK_CUR, 1) != 0) return luaL_error(L, "Unexpected EOF");

  // Читаем расширенные данные для чанка формата, если такие имеются
  if (nfsz > 16) {
    temp = 0;

    if (_readfile(L, &temp, 2, 1) != 2 || _seekfile(L, temp, SEEK_CUR, 1) != 0) return luaL_error(L, "Something wrong with the wave extension block");
  }
